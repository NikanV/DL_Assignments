...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  #############################################################################
  if use_loop:
    z = torch.stack([torch.mm(x[i], y[i]) for i in range(x.shape[0])])
  else:
    z = torch.bmm(x, y)
  #############################################################################
  #                            END OF YOUR CODE                               #
...
def normalize_columns(x):
  ...
  #                    TODO: Implement this function                          #
  #############################################################################
  cols_mean = x.sum(dim=0, keepdim=True) / x.shape[0]
  cols_std = ((x - cols_mean) ** 2).sum(dim=0, keepdim=True) / (x.shape[0] - 1)
  cols_var = cols_std.sqrt()
  y = (x - cols_mean) / cols_var
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
